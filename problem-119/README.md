...
You are building a routing system for electric vehicles. Given a list of charging station coordinates and a list of vehicle positions, find the nearest charging station for each vehicle.

- Coordinates are 2D integer pairs `[x, y]`.
- Distance is standard Euclidean distance (you may compare squared distances to avoid floating point).
- If two stations are equidistant from a vehicle, return the one with the smaller index in the original stations list.
